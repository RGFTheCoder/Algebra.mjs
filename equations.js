...
export class Equation {
  ...
  solveFor(variable) {
    ...
    if (this._isLinear() || this._variableCanBeIsolated(variable)) {
      let solvingFor = new Term(new Variable(variable));
      ...
      if (newLhs.terms.length === 0) {
        ...
        let discriminant = b.pow(2).subtract(a.multiply(c).multiply(4));
        ...
        let D0 = b.pow(2).subtract(a.multiply(c).multiply(3));
  ...
  _maxDegree() {
    let lhsMax = this.lhs._maxDegree();
    let rhsMax = this.rhs._maxDegree();
